...
public class Camera{
    ...
    private double width, height; // width and height of the resolution, and the vector to rotate
    private double nearPlane, farPlane; // near and far plane of the frustum, objects with t < near or t > far will not be rendered

    private double rotationY, rotationX;
    ...
    public Camera(Vector3D positionWorld, double width, double height, double nearPlane, double farPlane, double rotationY, double rotationX) {
        ...
        // Y rotation: turns the camera left/right
        Vector3D aimedY = baseAiming.rotateY(rotationY);
        Vector3D horizontalY = baseHorizontal.rotateY(rotationY);
        Vector3D verticalY = baseVertical; // Y rotation doesn't affect vertical

        // X rotation: tilts the camera up/down
        this.aiming = aimedY.rotateX(rotationX);
        this.horizontal = horizontalY; // X rotation doesn't affect horizontal
        this.vertical = verticalY.rotateX(rotationX);

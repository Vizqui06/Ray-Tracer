...
public class LightLambertian extends Light {
    ...
    // Shadow implementation (do not really apply neither for Lambertian nor Phong)

    @Override
    public Vector3D getDirectionToLight(Vector3D hitPoint) {
        // Directional light (Lambertian): direction is constant, independent of hit point
        // The stored direction points FROM the light TOWARD the scene, so it must be negated
        return direction.scalarIt(-1.0);
    }

    @Override
    public double getDistanceToLight(Vector3D hitPoint) {
        // Directional lights (Lambertian) are infinitely far ---> no distance cutoff for shadow rays

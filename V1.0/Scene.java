...
public class Scene {
    ...
    private Color shade (Intersection hit, Ray ray, int bounce, Color backgroundColor){
        ...
        if (refLEctivity > 0.0 && bounce < MAX_BOUNCES) {
            ...
            Vector3D reflectedOrigin = hit.getHitPoint().vectorAddition(normal.scalarIt(1e-3));
        ...
        if(refRActivity > 0.0 && bounce < MAX_BOUNCES){
            ...
            if(sinThetaSquare <= 1.0){
                ...
                Vector3D refractedOrigin = hit.getHitPoint().vectorSubstraction(refractNormal.scalarIt(1e-3));

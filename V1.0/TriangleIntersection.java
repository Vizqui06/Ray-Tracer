...
public class TriangleIntersection extends Object3D{
    ...
    public Intersection intersect(Ray ray) {
        ...
        // Normal map: if one is set, it overrides the shading normal computed above.
        // The normal map encodes directions in tangent space (local to the triangle surface).
        // To use it in lighting, it must be rotated into world space using the triangle's
        // TANGENT and BITANGENT vectors, which are derived from the UV layout.
        if (getNormalMap() != null && uv0 != null && uv1 != null && uv2 != null) {

            // Read the encoded normal direction from the normal map at the hit UV coords
            Vector3D mapNormal = getNormalMap().sampleNormal(hitU, hitV);

            if (mapNormal != null) {
                // Build the TANGENT from the edge v1v0 projected along the U axis of the UV layout.
                // deltaU1/deltaV1 are the UV deltas along the v0->v1 edge.
                // deltaU2/deltaV2 are the UV deltas along the v0->v2 edge.
                double deltaU1 = uv1.getX() - uv0.getX();
                double deltaV1 = uv1.getY() - uv0.getY();
                double deltaU2 = uv2.getX() - uv0.getX();
                double deltaV2 = uv2.getY() - uv0.getY();

                // The determinant scales the TANGENT and BITANGENT so they align with the UV axes exactly.
                // If it is near zero, the UVs are degenerate (collapsed), so, to avoid problems, the normal map is skipped.
                double normalDeterminant = deltaU1 * deltaV2 - deltaU2 * deltaV1;
                if (Math.abs(normalDeterminant) > 1e-6) {
                    double invDet2 = 1.0 / normalDeterminant;

                    // The formulas for Tangent and Bitangent are derived from the partial derivatives of the triangle's position with respect to the UV coordinates.

                    // TANGENT points in the direction of increasing U across the triangle surface
                    Vector3D Tangent = new Vector3D(
                        // The tangent vector is calculated using the UV deltas and the triangle edges (v1v0 and v2v0).
                        invDet2 * (deltaV2 * v1v0.getX() - deltaV1 * v2v0.getX()),
                        invDet2 * (deltaV2 * v1v0.getY() - deltaV1 * v2v0.getY()),
                        invDet2 * (deltaV2 * v1v0.getZ() - deltaV1 * v2v0.getZ())
                    ).normalization();

                    // BITANGENT points in the direction of increasing V, perpendicular to T on the surface
                    Vector3D Bitangent = new Vector3D(
                        // The bitangent vector is calculated similarly, but using the other set of UV deltas.
                        // Which are swapped and negated to ensure it is perpendicular to the tangent and correctly oriented with the UV layout.
                        invDet2 * (-deltaU2 * v1v0.getX() + deltaU1 * v2v0.getX()),
                        invDet2 * (-deltaU2 * v1v0.getY() + deltaU1 * v2v0.getY()),
                        invDet2 * (-deltaU2 * v1v0.getZ() + deltaU1 * v2v0.getZ())
                    ).normalization();

                    // Rotate the tangent-space normal into world space:
                    // world normal = mapNormal.x * TANGENT + mapNormal.y * BITANGENT + mapNormal.z * NORMAL (shading normal)
                    shadingNormal = new Vector3D(
                        mapNormal.getX() * Tangent.getX() + mapNormal.getY() * Bitangent.getX() + mapNormal.getZ() * shadingNormal.getX(),
                        mapNormal.getX() * Tangent.getY() + mapNormal.getY() * Bitangent.getY() + mapNormal.getZ() * shadingNormal.getY(),
                        mapNormal.getX() * Tangent.getZ() + mapNormal.getY() * Bitangent.getZ() + mapNormal.getZ() * shadingNormal.getZ()
                    ).normalization();
                }
            }
        }
